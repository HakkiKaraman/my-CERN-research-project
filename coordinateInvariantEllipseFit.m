...
n = size(dataPoints,1); 
%the first coloumn is x coordinates of the data
xCoordinateDataPoints = (dataPoints(:,1))';
%the second coloumn is y coordinates of the data
yCoordinateDataPoints = (dataPoints(:,2))'; 

%--- CORE ALGORITHM -------------------------------------------------------
%This part calculates M and N matrices by using the data (To understand the 
%mathematical foundation check the references) and solves Generalized 
%Eigenvalue problem (M*c == lambda*N*c). Smallest eigenvalue lambda and 
%corresponding eigenvector c which holds the ellipse condition gives the 
%solution. The first one is Tabubin's error metric i.e. Geometric Mean Square Error, the 
%second one is the coefficients of the quadric that represent optimal ellipse. 

%General ellipse equation c1 + c2*x + c3*y + c4*x^2 + c5*x*y + c6*y^2 == 0
%OR [c1 c2 c3 c4 c5 c6]*[1 x y x^2 x*y y^2]' == 0 or c'*X ==0

%matrixM == X'*X == [1; x; y; x^2; x*y; y^2]*[1; x; y; x^2; x*y; y^2]'
%The matrixM summation is calculated in vectorized form in stead of 
%'for loops' for computational performance, size of matrixM is (6,n)×(n,6)=(6,6)
...
%The matrixN summation is calculated in vectorized form in stead of 
%'for loops' for computational performance, size of matrixN is (6,2*n)×(2*n,6)=(6,6)
...
while matrixEigenVectors(5,k)^2-4*matrixEigenVectors(4,k)*matrixEigenVectors(6,k) >= 0 
    k = k - 1; 
    %if it was not an ellipse checks the eigenvalues in order till it finds the ellipse
...
Ellipse.taubinsErrorMetricVersion3 = ((matrixEigenVectors(:,k))'*matrixM*matrixEigenVectors(:,k))/...
                                     ((matrixEigenVectors(:,k))'*matrixN*matrixEigenVectors(:,k));
...
subplot(1,2,2); 
plot(xCoordinateDataPoints,yCoordinateDataPoints,'.b',Ellipse.xCoordinateCenter,...
     Ellipse.yCoordinateCenter,'+r'); %plotting the data points
hold on

text(Ellipse.xCoordinateCenter,Ellipse.yCoordinateCenter, sprintf('(%.2f,%.2f)',...
    Ellipse.xCoordinateCenter, Ellipse.yCoordinateCenter)) %ploting the center of the ellipse

plot(Ellipse.xCoordinateMajorAxisEndPoints,Ellipse.yCoordinateMajorAxisEndPoints,'-or'); 
%ploting the major axis of the ellipse

plot(Ellipse.xCoordinateMinorAxisEndPoints,Ellipse.yCoordinateMinorAxisEndPoints,'-or'); 
%ploting the minor axis of the ellipse

fimplicit(@(X,Y) coefficientsOfQuadric(1) + coefficientsOfQuadric(2)*X + coefficientsOfQuadric(3)*Y + ...
                 coefficientsOfQuadric(4)*X.^2 + coefficientsOfQuadric(5)*X.*Y + ...
                 coefficientsOfQuadric(6)*Y.^2, 'r'); %plotting the fitting ellipse

text((Ellipse.xCoordinateCenter + Ellipse.xCoordinateMajorAxisEndPoints(2))/2,...
     (Ellipse.yCoordinateCenter + Ellipse.yCoordinateMajorAxisEndPoints(2))/2,...
      sprintf('a = %.2f', Ellipse.lengthSemiMajorAxis')) %plotting semi major axis length
  
text((Ellipse.xCoordinateCenter + Ellipse.xCoordinateMinorAxisEndPoints(2))/2,...
     (Ellipse.yCoordinateCenter + Ellipse.yCoordinateMinorAxisEndPoints(2))/2,...
      sprintf('b = %.2f', Ellipse.lengthSemiMinorAxis')) %plotting semi minor axis length
  
text(Ellipse.xCoordinateMajorAxisEndPoints(1), Ellipse.yCoordinateMajorAxisEndPoints(1),...
     sprintf(' \\angle \\theta = %.1f\\circ',(Ellipse.obliqueAngleRadians*180/pi))); %plotting theta value

%saving the limits of the second subplot to use the same values for the
%first subplot
xlim_reference = xlim;
ylim_reference = ylim;

%printing Geometric RMSE value 
